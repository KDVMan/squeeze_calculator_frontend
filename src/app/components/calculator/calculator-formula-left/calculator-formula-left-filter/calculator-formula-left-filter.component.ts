...
@Component({
    selector: 'app-calculator-formula-left-filter',
    templateUrl: './calculator-formula-left-filter.component.html',
    standalone: true,
    imports: [ReactiveFormsModule, FormSelectComponent, SvgIconComponent]
})
export class CalculatorFormulaLeftFilterComponent implements OnInit, OnDestroy {
    ...
    constructor(private readonly formBuilder: FormBuilder,
        ...
        this.items = HelperService.convertEnumToKeyValue(CalculatorFieldEnum, 'filter', calculatorFieldExclusion.filter);
    }
    ...
    public onAdd(): void {
        if (!this.preset.filters) this.preset.filters = [];

        this.preset.filters.push({
            name: this.formGroup.get('filter').value,
            value: 0
        });
